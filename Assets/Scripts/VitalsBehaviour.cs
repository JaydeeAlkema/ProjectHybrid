...
public class VitalsBehaviour : MonoBehaviour
{
	...
	private void VitalsDecay()
	{
		...
		{
			hunger--;
			if(hunger < 25) NotificationsManager.SendNotification("1", "Pet Hunger Low!", "Your pet has run very low on food! Give your pet some food before it terrorizes your home!", System.DateTime.Now, default, default);
		}
		if(Random.Range(1, 100) < thirstDecayAmount && thirst > 0)
		{
			thirst--;
			if(thirst < 25) NotificationsManager.SendNotification("1", "Pet Thirst Low!", "Your pet has run very low on Water! Give your pet some Water before it terrorizes your home!", System.DateTime.Now, default, default);
		}
		if(Random.Range(1, 100) < energyDecayAmount && energy > 0)
		{
			if(energy < 15)
			{
				...
				NotificationsManager.SendNotification("1", "Pet Energy Low!", "Your pet has run very low on energy!", System.DateTime.Now, default, default);
			}
			energy--;
		}
		...
		{
			hygiene--;
			if(thirst < 25) NotificationsManager.SendNotification("1", "Pet Hygiene Low!", "Your pet is very filthy! Give him a wash or suffer the consequences!", System.DateTime.Now, default, default);
